-- =====================================================================
-- Reaplica a tolerância de 1 minuto antes da cobrança de excedente
-- (fa_kiosk_session_timing) e estende a mesma tolerância para Pacotes
-- (fa_checkout) — em todas as unidades.
-- =====================================================================
-- A migration 20260828000002_fa_excedente_tolerancia_1min.sql escreveu
-- essa tolerância para fa_kiosk_session_timing, mas nunca chegou a ser
-- aplicada no banco (não aparece em list_migrations — ficou só no
-- repositório, um caso a mais do drift já conhecido do projeto). Na
-- prática, todo fechamento de sessão de Plano cobrava o minuto excedente
-- sem nenhuma tolerância, mesmo a UI (packages/domain/session-timer.ts,
-- OVERAGE_GRACE_PERIOD_MS) já mostrando o card como "sem excedente"
-- nesse primeiro minuto — foi exatamente esse descompasso que cobrou
-- R$51 de uma sessão de 20 min que devia ter parado em R$48 (fechada 3,5s
-- depois do teto do plano).
--
-- Esta migration:
-- 1) Reaplica fa_kiosk_session_timing com a tolerância de 60.000ms,
--    igual ao motor de tempo do cliente (packages/domain).
-- 2) Estende a mesma tolerância (1 minuto) para o excedente de Pacotes
--    (Day Use, Porto Seguro etc.) em fa_checkout, que usa um cálculo
--    próprio (saldo consumido via fa_kiosk_package_consume) e nunca teve
--    tolerância nenhuma — pedido explícito: "tolerância de 1min, após
--    terminar o tempo do plano ou pacote em todas as unidades".
-- Banco de horas (uses_hour_bank) fica de fora de propósito: não foi
-- pedido e é um crédito à parte, não "tempo de plano/pacote" terminando.
-- =====================================================================
