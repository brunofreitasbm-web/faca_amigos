-- O fechamento das unidades continua às 22:00, mas a venda de novos
-- planos/pacotes/entradas de banco de horas agora para 15 minutos antes
-- (21:45) em todas as unidades, Circuito e Playground — em vez de ir até
-- o próprio horário de fechamento. A checagem antiga usava `v_remaining
-- <= 0` (ou seja, autorizava até 21:59); agora usa um corte fixo de
-- v_sales_cutoff_minutes, respeitado nas três origens de entrada (banco
-- de horas, pacote e plano avulso) dentro de fa_checkin.
...
declare
  ...
  v_closing_time text;
  v_remaining integer;
  v_sales_cutoff_minutes constant integer := 15;
...
begin
  ...
  if p_use_hour_bank then
    ...
    if v_remaining is not null and v_remaining < v_sales_cutoff_minutes then
      raise exception 'FORA_DO_HORARIO: %', 'O shopping está fechando — não é possível iniciar novas entradas nos últimos 15 minutos antes do fechamento';
    end if;
    ...
    if v_remaining is not null then
      if v_remaining < v_sales_cutoff_minutes then
        raise exception 'FORA_DO_HORARIO: %', 'O shopping está fechando — não é possível iniciar novos planos nos últimos 15 minutos antes do fechamento';
      end if;
      ...
      if v_plan_minutes > v_remaining
        ...
        raise exception 'FORA_DO_HORARIO: %', format('Este plano não cabe até o fechamento (faltam %s min)', v_remaining);
      end if;
    end if;
  end if;
