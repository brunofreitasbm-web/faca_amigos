-- =====================================================================
-- Cupom de desconto passa a valer também para Pacotes na Entrada
-- =====================================================================
-- Pedido: os cards de plano/pacote na Entrada (DAY USE, PORTO SEGURO etc.)
-- devem aplicar automaticamente -40% (público geral) ou -50% (criança
-- neurodivergente) — o efeito de auto-aplicar cupom em EntradaScreen já
-- existia para Planos avulsos, mas fa_checkin() descartava o cupom sempre
-- que p_package_id não era nulo (ver comentário removido abaixo:
-- "Cupom não se combina com... pacote").
--
-- Muda aqui:
--   1) Cupom agora se combina com Pacote (só continua incompatível com
--      Banco de Horas, que não tem valor de plano/pacote para descontar).
--   2) A base do desconto percentual passa a ser o preço do pacote
--      (v_pkg_price_cents) quando for esse o caso, em vez de v_plan.value_cents
--      (que ficaria null e quebraria o cálculo).
--   3) Cupom restrito a um Plano específico (allowed_plan_id) não se aplica
--      a Pacote — não faz sentido para esse tipo de cupom, e p_plan_id vem
--      nulo nesse fluxo.
--   4) fa_kiosk_guardian_packages.charged_cents passa a refletir o valor já
--      com desconto (o nome do campo é literalmente "quanto foi cobrado") —
--      price_cents continua com o preço cheio de catálogo, para histórico.
--
-- fa_checkout() não precisa mudar: já subtrai coupon_discount_cents da
-- linha de Pacote de forma genérica (mesmo código usado para Plano).
-- =====================================================================
...
begin
  ...
  -- com Plano e, agora, também com Pacote.
