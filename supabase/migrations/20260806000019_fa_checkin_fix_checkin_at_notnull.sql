-- fa_kiosk_sessions.checkin_at (timestamptz) é a coluna original da
-- tabela-stub (ver comentário na migration 04) e ficou NOT NULL sem
-- default; fa_checkin() só preenchia checkin_at_ms (bigint), então todo
-- check-in falhava com "null value in column checkin_at violates
-- not-null constraint" assim que o bug do cupom (migration anterior) foi
-- corrigido. Passa a gravar checkin_at também, derivado do mesmo v_now_ms.
...
begin
  ...
  insert into fa_kiosk_sessions (
    ...
    wristband_code, ticket_code, checkin_at, checkin_at_ms, checkin_by_employee_id,
    ...
    v_wristband, v_ticket, to_timestamp(v_now_ms / 1000.0), v_now_ms, p_employee_id,
