-- =====================================================================
-- Aluguel avulso de pelúcia no Playground — Passo 2 / 4: entrada
-- =====================================================================
-- Corpo idêntico ao vigente (conferido via pg_get_functiondef em
-- 2026-09-19 contra 20260910120200_fa_prepaid_session.sql), com o braço
-- do aluguel:
--   * `v_is_rental`: plano PLAYGROUND com asset_kind preenchido;
--   * reserva o ativo como o Circuito faz, validando tipo e unidade;
--   * criança já em sessão ATIVA -> CRIANCA_JA_EM_SESSAO;
--   * cupom não vale (CUPOM_NAO_APLICAVEL_PELUCIA) — nem o 40%
--     automático da tela;
--   * não conta visita de fidelidade;
--   * impressão só com o recibo (sem pulseira de entrada do Playground).
-- Mesma assinatura de 17 argumentos: `create or replace` substitui sem
-- overload, e os grants existentes permanecem.
-- =====================================================================
...
begin
  ...
  -- Aluguel de pelúcia não é entrada no Playground: sem pulseira, para a
  -- monitora da porta não confundir com uma criança liberada para brincar.
  if v_s.rental_kind is null then
...
begin
  ...
  if v_is_rental then
    ...
    if not exists (
      select 1 from fa_kiosk_assets
       where id = p_asset_id and unit_id = p_unit_id and kind = v_plan.asset_kind
    ) then
  ...
  insert into fa_kiosk_sessions (
    ...
    v_credit_min_consumption, case when v_is_rental then v_plan.asset_kind end
  );
