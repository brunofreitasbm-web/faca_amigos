-- Persistência permanente de Criança Neurodivergente e suporte à busca com inclusive_eligible

-- 1. Atualizar fa_kiosk_search_children para retornar inclusive_eligible
drop function if exists fa_kiosk_search_children(text, uuid);
...
returns table (
  ...
  visits_in_window integer, is_vip boolean, inclusive_eligible boolean
) as $$
  with base as (
    select c.id, c.full_name, c.birth_date, c.inclusive_eligible, g.phone_e164, g.full_name as guardian_name, g.cpf
     ...
     group by c.id, c.full_name, c.birth_date, c.inclusive_eligible, g.phone_e164, g.full_name, g.cpf
  ...
  select b.id, b.full_name, b.birth_date, b.phone_e164, b.guardian_name, b.cpf,
         ...
         coalesce(b.inclusive_eligible, false)
    from base b
$$ language sql stable;

grant execute on function fa_kiosk_search_children(text, uuid) to authenticated;

-- 2. Atualizar fa_checkin para gravar inclusive_eligible = true permanentemente em fa_kiosk_children
...
begin
  ...
  elsif coalesce((p_child->>'inclusiveEligible')::boolean, false) then
    update fa_kiosk_children
       set inclusive_eligible = true
     where id = v_child_id;
  end if;
