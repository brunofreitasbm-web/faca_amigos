...
-- Ocorre ao fazer check-in com pacotes (ex.: Day Use, Porto Seguro), pois v_plan ficava não atribuído
-- (SELECT INTO v_plan só roda quando p_package_id é null e p_use_hour_bank é false).
-- Ao tentar calcular v_discount_base_cents acessando v_plan.value_cents no branch ELSE do CASE,
-- o PL/pgSQL tentava resolver o registro não inicializado e lançava a exceção.
