-- Cupons "N ESTRELAS (PARQUE SHOPPING)" só valem no plano de 30 minutos do
-- Playground (Parque Shopping) — regra de negócio definida pela operação.
-- allowed_plan_id nulo = cupom vale para qualquer plano da unidade (mantém
-- o comportamento atual dos demais cupons).
alter table fa_kiosk_coupons
  add column if not exists allowed_plan_id uuid references fa_kiosk_plans (id);
...
-- Restringe os cupons "N ESTRELAS (PARQUE SHOPPING)" já cadastrados ao
-- plano de 30 minutos do Playground (Parque Shopping).
update fa_kiosk_coupons c
set allowed_plan_id = p.id
from fa_kiosk_plans p
where p.unit_id = c.unit_id
  and p.activity = 'PLAYGROUND'
  and p.duration_value = 30
  and p.duration_unit = 'MINUTO'
  and p.active
  and c.code in (
    '1 ESTRELA (PARQUE SHOPPING)',
    '2 ESTRELAS (PARQUE SHOPPING)',
    '3 ESTRELAS (PARQUE SHOPPING)'
  );
