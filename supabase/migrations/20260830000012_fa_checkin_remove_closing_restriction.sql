-- Remove por completo a trava de horário de fechamento no check-in:
-- novos planos, pacotes e entradas por banco de horas passam a ser
-- aceitos a qualquer momento, mesmo depois do horário de fechamento
-- configurado por unidade. Antes (20260830000008), v_remaining <= 0
-- bloqueava novas vendas; agora essa checagem é removida por completo.
--
-- v_remaining ainda é calculado (usado só para o rateio de banco de
-- horas/pacote) mas nunca mais impede o início de um plano/pacote/banco
-- de horas. Como o check-in agora pode acontecer com v_remaining
-- negativo (depois do fechamento), o rateio usa greatest(0, ...) para
-- não alocar minutos negativos.
