-- =====================================================================
-- Saldo pré-pago da criança — Débito no fechamento (Passo 5 / 8)
-- =====================================================================
-- Migration própria, separada da anterior de propósito: reverter o
-- caminho do dinheiro (aqui) não pode reverter o caminho da entrada
-- (fa_prepaid_session).
--
-- O branch novo entra ANTES do `else` do plano avulso — nunca depois —
-- porque esse `else` chama fa_kiosk_package_consume incondicionalmente
-- (deixa um pacote do responsável cobrir um plano pago à parte). Uma
-- sessão pré-paga caindo ali comeria o pacote do responsável ALÉM do
-- próprio saldo: débito duplo silencioso.
--
-- `child_credit_min_consumption_minutes` (congelado na sessão em
-- fa_checkin) é o piso: um crédito vindo de um Plano avulso não
-- fraciona abaixo de uma visita inteira, igual a um Plano normal se
-- comportaria (sai em 10 min de um plano de 60, paga os 60 mesmo
-- assim). Vindo de um Pacote, o piso é null -> greatest(elapsed, 0) =
-- elapsed, fraciona livre. fa_kiosk_child_credit_consume() já limita
-- ao que existe de saldo, então pedir mais do que resta nunca debita
-- além da conta.
--
-- Tolerância de 1 minuto (`- 1`) igual à do Pacote: o crédito herda a
-- regra da origem (decisão do dono, 2026-09-10), e o Pacote já tem essa
-- tolerância. O branch do Banco de Horas, três linhas acima, NÃO tem
-- essa tolerância — é uma divergência pré-existente entre o motor SQL
-- e o motor TS (computeSessionTiming, que aplica a tolerância a todo
-- mundo, banco de horas incluso), documentada mas fora do escopo desta
-- migration: mexer nela aqui misturaria um bug antigo não relacionado
-- com esta feature nova.
-- =====================================================================
