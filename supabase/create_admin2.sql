-- Script para criar/garantir o colaborador "Admin 2" (admin2) com PIN 000000 no Supabase.
...
DECLARE
  ...
  v_emp_id uuid;
BEGIN
  -- 1. Obter ou criar usuário admin2 em auth.users
  SELECT id INTO v_user_id
  FROM auth.users
  WHERE email = 'admin2@kiosk.internal'
  LIMIT 1;
  ...
  -- 2. Obter ou criar colaborador admin2 em fa_kiosk_employees
  SELECT id INTO v_emp_id
  FROM fa_kiosk_employees
  WHERE full_name = 'Admin 2' OR auth_user_id = v_user_id
  LIMIT 1;

  IF v_emp_id IS NULL THEN
    INSERT INTO fa_kiosk_employees (auth_user_id, full_name, role, active)
    VALUES (v_user_id, 'Admin 2', 'ADMIN', true)
    RETURNING id INTO v_emp_id;
  ELSE
    UPDATE fa_kiosk_employees
    SET auth_user_id = v_user_id, full_name = 'Admin 2', role = 'ADMIN', active = true
    WHERE id = v_emp_id;
  END IF;
  ...
  RAISE NOTICE 'Admin 2 com PIN 000000 configurado com sucesso! ID do colaborador: %', v_emp_id;
END $$;
