...
export const acompanharPlanoSchema = z.object({
  ...
  assetKind: z.enum(["CARRO", "PELUCIA"]).nullable().optional().catch(null).transform((v) => v ?? null),
});
...
export const acompanharSessaoSchema = z.discriminatedUnion("status", [
  ...
  z.object({
    ...
    sessionId: uuidV7,
    childFirstName: z.string(),
    activity: z.enum(["PLAYGROUND", "CARRINHO"]).optional().default("PLAYGROUND").catch("PLAYGROUND"),
