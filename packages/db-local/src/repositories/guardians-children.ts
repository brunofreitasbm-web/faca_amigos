...
export function insertGuardian(db: Db, g: Omit<GuardianRow, "created_at_ms">, nowMs: number): void {
  db.prepare(
    `INSERT INTO guardians (id, full_name, phone_e164, created_at_ms) VALUES (@id, @full_name, @phone_e164, @created_at_ms)`,
  ).run({ ...g, created_at_ms: nowMs });
}
 ...
 */
export function searchChildrenByNameOrPhone(db: Db, query: string, limit = 10): (ChildRow & { phone_e164: string | null })[] {
  const like = `%${query}%`;
  return db
    .prepare(
      `SELECT c.*, g.phone_e164 AS phone_e164
       FROM children c
       LEFT JOIN child_guardians cg ON cg.child_id = c.id
       LEFT JOIN guardians g ON g.id = cg.guardian_id
       WHERE c.full_name LIKE ? OR g.phone_e164 LIKE ?
       GROUP BY c.id
       ORDER BY c.full_name
       LIMIT ?`,
    )
    .all(like, like, limit) as unknown as (ChildRow & { phone_e164: string | null })[];
}
